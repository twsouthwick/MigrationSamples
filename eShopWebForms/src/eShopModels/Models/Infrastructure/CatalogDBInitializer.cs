...
namespace eShopWebForms.Models.Infrastructure
{
    public class CatalogDBInitializer : CreateDatabaseIfNotExists<CatalogDBContext>
    {
        ...
        private const string CatalogItemHiLoSequenceScript = @"eShopModels.Models.Infrastructure.dbo.catalog_hilo.Sequence.sql";
        private const string CatalogBrandHiLoSequenceScript = @"eShopModels.Models.Infrastructure.dbo.catalog_brand_hilo.Sequence.sql";
        private const string CatalogTypeHiLoSequenceScript = @"eShopModels.Models.Infrastructure.dbo.catalog_type_hilo.Sequence.sql";
        ...
        {
            using (var stream = typeof(CatalogDBContext).Assembly.GetManifestResourceStream(scriptFile))
            {
                if (stream is null)
                {
                    throw new FileNotFoundException($"Could not file {scriptFile}");
                }

                using (var reader = new StreamReader(stream))
                {
                    var contents = reader.ReadToEnd();
                    context.Database.ExecuteSqlCommand(contents);
                }
            }
        }
